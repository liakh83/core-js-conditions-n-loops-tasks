...
function getBalanceIndex(arr) {
  ...
  let sumLeftArr = 0;
  for (let i = 0; i < arr.length; i += 1) {
    if (sumLeftArr === sumArr - sumLeftArr - arr[i]) {
      return i;
    }
    sumLeftArr += arr[i];
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    ...
    for (let j = 0; j < l; j += 1) {
      if (j % 2 === 0) {
        evenIndex += shuffleStr[j];
      } else {
        oddIndex += shuffleStr[j];
      }
    }
    shuffleStr = evenIndex + oddIndex;
    arrayStr[i] = shuffleStr;
    if (shuffleStr === str) {
      return arrayStr[(iterations % (i + 1)) - 1];
    }
  }
  return shuffleStr;
}
...
function getNearestBigger(number) {
  ...
  let j = l - 1;
  while (digitsArray[i] >= digitsArray[j]) {
    j -= 1;
  }

  [digitsArray[i], digitsArray[j]] = [digitsArray[j], digitsArray[i]];

  const leftDigits = digitsArray.splice(0, i + 1);
  const rightDigits = [...digitsArray].sort((a, b) => a - b);
  const result = Number([...leftDigits, ...rightDigits].join(''));
  return result;
}
